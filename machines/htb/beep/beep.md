...
- Elastix vtiger 5.1.0 LFI 
LFI get asterisk config file with clear passwd, same password for ssh root


# nmap

```
# nmap -A -sV -sC -p-  10.10.10.7
Starting Nmap 7.70 ( https://nmap.org ) at 2019-09-03 23:30 CEST
...
|_pop3-capabilities: LOGIN-DELAY(0) TOP IMPLEMENTATION(Cyrus POP3 server v2) STLS USER AUTH-RESP-CODE PIPELINING RESP-CODES UIDL APOP EXPIRE(NEVER)
...
|_imap-capabilities: Completed OK URLAUTHA0001 RENAME X-NETSCAPE NAMESPACE ID LIST-SUBSCRIBED LISTEXT IMAP4 ATOMIC UNSELECT THREAD=ORDEREDSUBJECT SORT IDLE THREAD=REFERENCES CONDSTORE MULTIAPPEND UIDPLUS LITERAL+ CATENATE ANNOTATEMORE SORT=MODSEQ MAILBOX-REFERRALS STARTTLS NO BINARY QUOTA IMAP4rev1 CHILDREN ACL RIGHTS=kxte
...
|_ssl-date: 2019-09-03T21:26:26+00:00; -7m42s from scanner time.
...
![](images/beep_elastix.png)
# Elastix vtiger 5.1.0 LFI 

Elastix uses vtiger CRM 5.1.0
...
aeff3def0c765c2677b94715cffa73ac Sorry! Attempt to access restricted file.
...
Creds => admin:jEhdIekWmdjE

# Creds for https://10.10.10.7/

Go to
https://10.10.10.7/index.php?menu=tools
Asterisk cli : core show help
Util ???


# Same passwd for ssh root
```
# ssh root@10.10.10.7
